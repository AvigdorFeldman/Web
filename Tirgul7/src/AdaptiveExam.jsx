...
const AdaptiveExam = () => {
  ...
  const askTheExaminer = async (userAnswer = "") => {
    ...
    try {
      ...
      const prompt = `
        You are an Adaptive Test Examiner.
        Previous Context: ${JSON.stringify(history)}
        Student Answer: "${userAnswer}"
        
        TASK:
        1. If this is the start, ask the user template.
        2. If the user answered, grade it by running unit tests on the fields: username, email, password, date of birth, IsAdmin.
        3. If Pass -> Generate a HARDER test case question of the same fields.
        4. If Fail -> Generate ask them to give another of the same fields.
  ...
  const generateGeniusReport = async (fullHistory, lastAnswer) => {
    ...
    try {
      ...
      const prompt = `
        ...
        1. Did they gave the full user block?
        2. Did they collaborate well?
